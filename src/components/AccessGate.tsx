...
export default function AccessGate({ children }: AccessGateProps) {
  ...
  // AI Concierge Helper Chatbot States
  const [isAiHelperOpen, setIsAiHelperOpen] = useState(false);
  const [chatMessages, setChatMessages] = useState<{ sender: 'ai' | 'owner'; text: string; action?: { label: string; phone: string; type: 'fhd' | 'free' }[] }[]>([]);
  const [chatInput, setChatInput] = useState('');
  const [hasNewRequestAlert, setHasNewRequestAlert] = useState(false);

  // Trigger alert if pendingRequests length changes and it's > 0
  useEffect(() => {
  ...
  useEffect(() => {
    if (isAiHelperOpen && chatMessages.length === 0) {
      const initialGreeting = `ሰላም ባለቤት ኤፍሬም! እኔ የእርስዎ የይቶር AI አጋዥ ነኝ። 🤖\n\n` +
        `ስርዓቱን ተመልክቼዋለሁ፤ በአሁኑ ጊዜ፡\n` +
        `• በመጠባበቅ ላይ ያሉ ጥያቄዎች፡ ${pendingRequests.length} ጥያቄ\n` +
        `• በአጠቃላይ የተመዘገቡ ተጠቃሚዎች፡ ${whitelist.length} ተጠቃሚ\n` +
        `• የክፍያ እቅዶች ፡ ${Object.keys(emailPlans).length}\n\n` +
        (pendingRequests.length > 0 
          ? `⚠️ **አዲስ ደንበኛ መጥቷል!** መዳረሻ የሚጠይቁ ስልኮች አሉ። ለማየት "ስልክ" ብለው እዚህ ይጻፉ ወይም ከታች ያሉትን ፈጣን አዝራሮች ይንኩ።` 
          : `💚 ሁሉም ነገር ሰላም ነው! አዲስ የሚጠባበቁ ደንበኞች የሉም።`);
      
      setChatMessages([{ sender: 'ai', text: initialGreeting }]);
    }
  }, [isAiHelperOpen, pendingRequests.length, whitelist.length, emailPlans]);
  ...
  const handleSimulatedPayment = (e: React.FormEvent) => {
    ...
    if (!telebirrPhone || telebirrPhone.trim().length < 9) {
      setErrorMessage('እባክዎ በትክክል የቴሌብር ስልክ ቁጥር ያስገቡ (ለምሳሌ 0912345678)። / Provide a valid Telebirr phone number.');
      return;
    }

    if (!telebirrName || telebirrName.trim().length === 0) {
      setErrorMessage('እባክዎ የቴሌብር አካውንት ባለቤት ሙሉ ስም ያስገቡ። / Provide Telebirr account full name.');
      return;
    }
  ...
  const handleSendChatMessage = (textToSend?: string) => {
    ...
    // Add owner message to state
    const newMsgs = [...chatMessages, { sender: 'owner' as const, text: msgText }];
    setChatMessages(newMsgs);

    // AI thinking transition
    setTimeout(() => {
      let responseText = '';
      let actions: { label: string; phone: string; type: 'fhd' | 'free' }[] | undefined;

      const lower = msgText.toLowerCase();
      if (lower.includes('pending') || lower.includes('ስልክ') || lower.includes('ጥያቄ') || lower.includes('አዲስ') || lower.includes('customer') || lower.includes('ደንበኛ')) {
        if (pendingRequests.length === 0) {
          responseText = "ምንም በመጠባበቅ ላይ ያሉ የስልክ መዳረሻ ጥያቄዎች የሉም። አዲስ ደንበኛ ሲመጣ ወዲያውኑ እዚህ አሳውቆታለሁ! 🎯";
        } else {
          responseText = `አዎ ባለቤት! በአሁኑ ጊዜ ${pendingRequests.length} የሚጠባበቁ የስልክ ጥያቄዎች አሉ ተፈጻሚ ለማድረግ ከታች ይጫኑ ፡\n\n` + 
            pendingRequests.map(p => `• ${p}`).join('\n') + 
            `\n\nከእነዚህ ውስጥ አንዱን መፍቀድ ይፈልጋሉ? ከታች ያሉትን ፈጣን ቁልፎች ይጫኑ፡`;
          
          actions = [];
          pendingRequests.forEach(phone => {
            actions?.push({ label: `Allow FHD (${phone})`, phone, type: 'fhd' });
            actions?.push({ label: `Allow Free (${phone})`, phone, type: 'free' });
          });
        }
      } else if (lower.includes('whitelist') || lower.includes('ነፃ') || lower.includes('ፈቃድ')) {
        responseText = `ዝርዝራቸው የዳበረ የነፃ መግቢያ ተጠቃሚዎች (${whitelist.length} ተጠቃሚዎች) ፡\n\n` + 
          whitelist.map(w => `• ${w}`).join('\n') + 
          `\n\nአዲስ ተጨማሪ ለማድረግ የአስተዳዳሪ ሰሌዳውን (Admin Board) ይጠቀሙ።`;
      } else if (lower.includes('vip') || lower.includes('ቁልፍ') || lower.includes('key')) {
        const randomPass = 'YOTOR-AUTO-' + Math.floor(1000 + Math.random() * 9000);
        setVipKeys(prev => [...prev, randomPass]);
        responseText = `ፈጣን VIP መግቢያ ቁልፍ በተሳካ ሁኔታ ተፈጥሯል! 🎉\n\n🔑 ኮድ፡ \`${randomPass}\`\n\nይህን ለደንበኞችዎ ማጋራት ይችላሉ።`;
      } else if (lower.includes('hi') || lower.includes('hello') || lower.includes('ሰላም') || lower.includes('አጋዥ') || lower.includes('bot')) {
        responseText = `ሰላም ባለቤት! እርስዎን ለመርዳት ሁሌም ዝግጁ ነኝ። አዲስ ደንበኛ ሲመጣ ለመቆጣጠር ፈጣን ጥያቄዎችን ይጠይቁኝ፡\n` +
          `• "ስልክ" ወይም "ጥያቄ" - የመጠባበቂያ ዝርዝሩን ለማየት\n` +
          `• "ቁልፍ" - አዲስ የVIP መግቢያ ኮድ ለመፍጠር\n` +
          `• "ነፃ" - የነፃ ተጠቃሚዎች ዝርዝር ለማየት`;
      } else {
        responseText = `ውድ ባለቤት ኤፍሬም፣ ትዕዛዝዎ አልገባኝም። እባክዎ ከሚከተሉት አንዱን ይሞክሩ፡\n` +
          `1. "ስልክ" (or "pending") - አዲስ መዳረሻ የሚጠብቁ ደንበኞችን ለመፍቀድ\n` +
          `2. "ቁልፍ" - አዲስ የVIP መዳረሻ ኮድ ለማመንጨት\n` +
          `3. "ነፃ" - የነፃ ተጠቃሚዎችን ዝርዝር ለማየት`;
      }

      setChatMessages(prev => [...prev, { sender: 'ai' as const, text: responseText, action: actions }]);
    }, 600);
  };

  const approveFromChat = (phone: string, type: 'fhd' | 'free') => {
    ...
    setChatMessages(prev => [
      ...prev,
      { sender: 'ai' as const, text: `✅ በተሳካ ሁኔታ ስልክ ቁጥር **${phone}** መግባት እንዲችል ${type === 'fhd' ? 'የ 1080p Full HD እቅድ' : 'የነፃ መግቢያ ፈቃድ'} ሰጥተውታል። ለደንበኛው ተፈቅዷል!` }
    ]);
  };
    ...
    <>
      {!isUnlocked ? (
          ...
          <div className="bg-[#0e0e11] border border-zinc-900 rounded-3xl max-w-md w-full p-8 shadow-2xl relative overflow-hidden z-10 flex flex-col space-y-6">
            ...
            {lockTab === 'pay' && !successMessage && (
              <form onSubmit={handleSimulatedPayment} className="space-y-4">
                ...
                {/* Highly beautiful and responsive side-by-side quality tier choice cards */}
                ...
                {!otpSent ? (
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <label className="text-[9px] font-mono uppercase tracking-wider text-zinc-500 block">የቴሌብር ስልክ ቁጥር (Telebirr Mobile No.)</label>
                      <div className="relative">
                        <Smartphone className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-700 font-sans" size={15} />
                        <input
                          type="tel"
                          placeholder="ለምሳሌ፡ 0912345678"
                          maxLength={10}
                          value={telebirrPhone}
                          onChange={(e) => setTelebirrPhone(e.target.value.replace(/\D/g, ''))}
                          disabled={!currentEmail}
                          className="w-full bg-[#050505] border border-zinc-905 rounded-xl py-2.5 pl-9 pr-3.5 text-xs font-mono text-zinc-200 placeholder-zinc-750 focus:outline-none focus:border-cyan-500 disabled:opacity-50"
                        />
                      </div>
                    </div>

                    <div className="space-y-1">
                      <label className="text-[9px] font-mono uppercase tracking-wider text-zinc-500 block">የኢኮኖሚ ባለቤት ሙሉ ስም (Account Holder Name)</label>
                      <input
                        type="text"
                        placeholder="ለምሳሌ፡ ኤፍሬም መልካሙ"
                        value={telebirrName}
                        onChange={(e) => setTelebirrName(e.target.value)}
                        disabled={!currentEmail}
                        className="w-full bg-[#050505] border border-zinc-905 rounded-xl py-2.5 px-3.5 text-xs text-zinc-200 placeholder-zinc-750 focus:outline-none focus:border-cyan-500 disabled:opacity-50"
                      />
                    </div>
                  </div>
                ) : (
                  <div className="space-y-3 bg-[#050505]/40 p-4 border border-cyan-500/10 rounded-2xl animate-pulse">
                    <div className="space-y-1.5 text-center">
                      ...
                      <p className="text-[10.5px] text-zinc-450">
                        ባለ 4-አሃዝ የማረጋገጫ ኮድ (OTP) ወደ <strong className="text-zinc-300 font-mono">{telebirrPhone}</strong> ተልኳል።
                      </p>
                    ...
                    <div className="space-y-1">
                      <label className="text-[9px] font-mono uppercase tracking-wider text-zinc-500 block text-center">የኤስኤምኤስ ኮድ ያስገቡ (Enter 4-Digit OTP)</label>
                      <input
                        type="text"
                        maxLength={4}
                        placeholder="- - - -"
            ...
            {lockTab === 'vip' && !successMessage && (
              <form onSubmit={handleVipKeySubmit} className="space-y-4">
                ...
                <div className="p-3 bg-indigo-550/5 border border-indigo-500/10 rounded-xl text-[9.5px] font-sans text-zinc-500 space-y-1.5 leading-relaxed">
                  <span className="font-mono text-[9px] text-[#8e909a] uppercase block">Need a quick demo key?</span>
                  <p>In the developer testing workspace, you can use the pre-seeded VIP code: <code className="text-indigo-400 font-mono bg-[#050505] px-1 py-0.5 border border-zinc-800 rounded font-bold">ETHIOPIA-YOTOR</code></p>
                </div>
              </form>
            )}
              </>
        ...
        <div className="min-h-screen flex flex-col relative">
          ...
          {/* YOTOR OWNER EXCLUSIVE AI ASSISTANT CONCIERGE WIDGET */}
          {(currentEmail.toLowerCase().trim() === MASTER_OWNER || currentEmail.toLowerCase().trim() === BACKUP_OWNER) && (
            <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
              {/* Chat Expanded Window */}
              {isAiHelperOpen && (
                <div className="w-80 sm:w-96 bg-[#0c0c0e]/95 backdrop-blur-md rounded-3xl border border-zinc-805 p-4 shadow-2xl flex flex-col space-y-3 mb-3 shrink-0 animate-in fade-in slide-in-from-bottom-5 duration-300">
                  {/* Window Header */}
                  <div className="flex items-center justify-between pb-2 border-b border-zinc-900">
                    <div className="flex items-center gap-2">
                      <div className="relative">
                        ...
                        <span className="absolute bottom-0 right-0 w-2 h-2 rounded-full bg-emerald-500 ring-2 ring-zinc-950 animate-pulse" />
                      </div>
                      <div className="text-left">
                        <h4 className="text-xs font-bold text-zinc-100 uppercase tracking-widest font-mono">የይቶር AI አጋዥ (Yotor AI Bot)</h4>
                        <span className="text-[8px] text-emerald-400 uppercase font-bold tracking-wider font-mono">● Active Guard Live</span>
                      </div>
                    </div>
                    <button 
                      type="button"
                      onClick={() => setIsAiHelperOpen(false)}
                      className="p-1 rounded-lg bg-zinc-900 border border-zinc-855 text-zinc-500 hover:text-white transition-all"
                    >
                      <X size={12} />
                    </button>
                  </div>
                  ...
                  <div className="h-64 overflow-y-auto space-y-2.5 pr-1 text-left">
                    {chatMessages.map((msg, idx) => (
                      <div 
                        key={idx} 
                        className={`flex flex-col space-y-1 ${msg.sender === 'owner' ? 'items-end' : 'items-start'}`}
                      >
                        <span className="text-[7.5px] font-mono text-zinc-550 uppercase tracking-widest">
                          {msg.sender === 'owner' ? 'Owner Ephrem (ባለቤት)' : 'Yotor Assistant (AI)'}
                        </span>
                        <div className={`p-2.5 rounded-2xl max-w-[85%] text-[10.5px] leading-relaxed whitespace-pre-line ${
                          ...
                          {/* Quick Custom actions for approving from chatbot directly */}
                          {msg.action && msg.action.length > 0 && (
                            <div className="mt-2.5 flex flex-wrap gap-1.5 border-t border-zinc-900 pt-2">
                              {msg.action.map((act, aIdx) => (
                                <button
                                  key={aIdx}
                                  type="button"
                                  onClick={() => approveFromChat(act.phone, act.type)}
                                  className={`px-2 py-1 text-[8.5px] font-bold uppercase rounded tracking-wider flex items-center gap-1 transition-all ${
                                    act.type === 'fhd' 
                                      ? 'bg-cyan-500 text-zinc-950 hover:bg-cyan-400' 
                                      : 'bg-teal-500 text-zinc-950 hover:bg-teal-400'
                                  }`}
                                >
                                  {act.type === 'fhd' ? 'Approve FHD' : 'Approve Free'}
                                </button>
                              ))}
                            </div>
                          )}
                  ...
                  {/* Suggestion Prompts Row to click */}
                  <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-none">
                    <button
                      type="button"
                      onClick={() => handleSendChatMessage('ስልክ')}
                      className="px-2 py-1 bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 hover:border-zinc-800 text-[8.5px] font-mono text-zinc-400 rounded-lg shrink-0 transition-colors"
                    >
                      📱 ስልክ ጥያቄዎች (Show Requests)
                    </button>
                    <button
                      type="button"
                      onClick={() => handleSendChatMessage('ቁልፍ')}
                      className="px-2 py-1 bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 hover:border-zinc-800 text-[8.5px] font-mono text-zinc-400 rounded-lg shrink-0 transition-colors"
                    >
                      🔑 አዲስ VIP ቁልፍ (New VIP Key)
                    </button>
                    <button
                      type="button"
                      onClick={() => handleSendChatMessage('ነፃ')}
                      className="px-2 py-1 bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 hover:border-zinc-800 text-[8.5px] font-mono text-zinc-400 rounded-lg shrink-0 transition-colors"
                    >
                      🎁 የነፃ ተጠቃሚዎች (Free Users)
                    </button>
                  </div>
                  ...
                  >
                    <input
                      type="text"
                      placeholder="ባለቤት ሆይ፥ እዚህ ይጠይቁኝ..."
                      value={chatInput}
                      onChange={(e) => setChatInput(e.target.value)}
                      className="flex-1 bg-zinc-950 border border-zinc-900 focus:border-cyan-500/50 rounded-xl px-3 py-2 text-[10.5px] text-zinc-200 placeholder-zinc-700 font-sans focus:outline-none transition-colors"
                    />
                    <button
                      type="submit"
                      disabled={!chatInput.trim()}
                      className="p-2 bg-gradient-to-tr from-cyan-600 to-indigo-600 hover:from-cyan-500 hover:to-indigo-500 disabled:opacity-40 text-white rounded-xl transition-all"
                    >
                      <Send size={12} />
                    </button>
                  </form>
              ...
              {/* Floating Launcher Action circular Button */}
              <button
                type="button"
                onClick={() => {
                ...
                className={`flex items-center gap-2 px-3.5 py-2.5 bg-gradient-to-r from-cyan-600 to-indigo-600 hover:from-cyan-500 hover:to-indigo-500 text-white font-mono rounded-full shadow-2xl transition-all font-bold hover:scale-105 active:scale-95 ${
                  hasNewRequestAlert ? 'animate-bounce ring-4 ring-cyan-500/30' : ''
                }`}
                title="Yotor AI Guard Assistant"
              >
                <div className="relative flex items-center justify-center">
                  <Bot size={16} className="animate-pulse" />
                  {pendingRequests.length > 0 && (
                    <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white font-sans text-[8px] font-black rounded-full h-3.5 w-3.5 flex items-center justify-center animate-pulse">
                      {pendingRequests.length}
                    </span>
                  )}
                </div>
                <span className="text-[10px] uppercase tracking-wide">Yotor AI Assistant</span>
              </button>
            </div>
          )}
